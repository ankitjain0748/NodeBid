...
console.log(req.body)
        ...
        // Find the relevant Panna and Sangam models
        const Pannamodel = await Panna.find({ marketId }).populate('userId').populate('marketId');
        if (!Pannamodel || Pannamodel.length === 0) {
            return res.status(404).json({ message: "No Panna models found." });
        }
        ...
        let openPanna = null;
        let closePanna = null;
        console.log(openPanna , closePanna  )
        let resultData = {
            session,
            number,
            betdate,
...
console.log("resultData",resultData)
        ...
        // Check Panna models for open or close session
        for (const panna of Pannamodel) {
            if ((session === 'open' && panna.status === true) || (session === 'close' && panna.status === false)) {
                if (panna.point === sumOfDigits) {
